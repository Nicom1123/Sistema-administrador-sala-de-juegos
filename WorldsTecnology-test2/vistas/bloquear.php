...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['id']) && isset($_GET['table'])) {
    ...
    // Instanciar el controlador y ejecutar la función de bloqueo
    $controller = new $controllerName($mysqli);
    if ($controller->bloquear($id)) {
        header("Location: Console.php"); // Cambia la redirección si es necesario
        exit();
    } else {
        echo "Error: No se pudo bloquear el registro.";
    }
} else {
